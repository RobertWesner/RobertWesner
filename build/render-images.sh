...
(
    ...
    magick convert -size "${WIDTH}x${HEIGHT}" xc:none -draw "roundrectangle 0,0,$WIDTH,$HEIGHT,$ROUND,$ROUND" mask.png
    for filename in $(find img -name '*.html'); do
        ...
        magick convert $image -matte mask.png -compose DstIn -composite $image
        mogrify -strip $image
    done
)
